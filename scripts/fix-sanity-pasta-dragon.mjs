...
async function fix() {
  console.log('Pobieranie produktów ze słowem "Pasta do rąk" w nazwie, które są w złej kategorii...');
  const products = await client.fetch(`*[_type == "product" && name match "Pasta do rąk*" && category->slug.current == "elementy-wykonczeniowe-do-mocowania-sufitow-podwieszanych"]{
    _id, name
  }`);
  
  console.log(`Znaleziono ${products.length} past do rąk w złej kategorii.`);
  ...
  const mutations = products.map(p => {
    return {
      patch: {
        id: p._id,
        set: {
          category: { _type: 'reference', _ref: 'cat-srodki-czyszczaco-pielegnacyjne' },
  ...
  console.log('Zakończono przenoszenie past do rąk do właściwej kategorii.');
}
