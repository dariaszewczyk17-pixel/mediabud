...
async function fix() {
  ...
  for (const p of products) {
    ...
    if (name.includes('tabletki ceresit') || name.includes('tabletki do pochłaniacz')) {
      newCatSlug = 'srodki-czyszczaco-pielegnacyjne';
      newRootSlug = 'chemia-budowlana';
    } else if (name.includes('belka betonowa') || name.includes('ściana oporowa')) {
      newCatSlug = 'galanteria-betonowa';
      newRootSlug = 'stropy-i-sciany';
    } else if (name.includes('panel aluminiowy') || name.includes('tawerszyna') || name.includes('sufit raster')) {
      newCatSlug = 'plyty-sufitowe-metalowe';
      newRootSlug = 'sufity-podwieszane';
    } else if (name.includes('pistolet profi')) {
      newCatSlug = 'nawadnianie';
      newRootSlug = 'narzedzia-i-mocowania';
    } else if (name.includes('klej mineralny')) {
      newCatSlug = 'kleje-do-glazury';
      newRootSlug = 'chemia-budowlana';
    } else if (name.includes('listwa przyokienna')) {
      newCatSlug = 'narozniki-do-tynkow-mokrych';
      newRootSlug = 'sucha-zabudowa';
    } else if (name.includes('płyta akustyczna')) {
      newCatSlug = 'plyty-gipsowo-kartonowe';
      newRootSlug = 'sucha-zabudowa';
    } else if (name.includes('ościeżnica')) {
      newCatSlug = 'drzwi-i-akcesoria-do-drzwi';
      newRootSlug = 'stolarka-otworowa';
    } else if (name.includes('siatka zgrzewana')) {
      newCatSlug = 'siatki-elewacyjne';
      newRootSlug = 'izolacje-fasadowe';
    } else if (name.includes('wpust dachowy') || name.includes('wpust attykowy')) {
      newCatSlug = 'akcesoria-do-systemow-rynnowych';
      newRootSlug = 'dachy';
    } else if (name.includes('kaptur foliowy')) {
      newCatSlug = 'folie-ochronne';
      newRootSlug = 'narzedzia-i-mocowania';
    } else if (name.includes('regał/szafka')) {
      newCatSlug = 'wiadra-i-pojemniki-budowlane';
      newRootSlug = 'narzedzia-i-mocowania';
    } else if (name.includes('przegroda szklana')) {
      newCatSlug = 'okna-dachowe-i-akcesoria';
      newRootSlug = 'dachy';
    } else if (name.includes('litera') || name.includes('uchwyt do flag')) {
      newCatSlug = 'elementy-mocujace-uniwersalne';
      newRootSlug = 'narzedzia-i-mocowania';
    } else if (name.includes('opona gumowa')) {
      newCatSlug = 'narzedzia-reczne';
      newRootSlug = 'narzedzia-i-mocowania';
    } else if (name.includes('drut boll')) {
      newCatSlug = 'narzedzia-do-ciecia';
      newRootSlug = 'narzedzia-i-mocowania';
    }
