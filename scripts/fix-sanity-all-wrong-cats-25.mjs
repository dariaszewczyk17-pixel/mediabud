...
async function fix() {
  ...
  for (const p of products) {
    ...
    if (name.includes('torba foliowa')) {
      newCatSlug = 'worki-na-gruz-i-smieci';
      newRootSlug = 'narzedzia-i-mocowania';
    } else if (name.includes('podbitka')) {
      newCatSlug = 'plyty-drewnopochodne'; // Zmiana na istniejącą kategorię
      newRootSlug = 'stropy-i-sciany';
    } else if (name.includes('piasek kwarcowy')) {
      newCatSlug = 'dodatki-do-zapraw-i-betonu'; // Zmiana na istniejącą kategorię
