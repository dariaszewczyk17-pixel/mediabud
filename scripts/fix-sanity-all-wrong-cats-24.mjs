...
async function fix() {
  ...
  for (const p of products) {
    ...
    if (name.includes('gąbka ścierna') || name.includes('płótno ścierne')) {
      newCatSlug = 'artykuly-scierne';
      newRootSlug = 'narzedzia-i-mocowania';
    } else if (name.includes('paca22') || name.includes('wałek mal') || name.includes('zapas 18') || name.includes('szablon')) {
      newCatSlug = 'narzedzia-malarskie';
      newRootSlug = 'narzedzia-i-mocowania';
    } else if (name.includes('spodnie')) {
      newCatSlug = 'odziez-ochronna';
      newRootSlug = 'narzedzia-i-mocowania';
    } else if (name.includes('pistolet do kleju')) {
      newCatSlug = 'elektronarzedzia';
      newRootSlug = 'narzedzia-i-mocowania';
    } else if (name.includes('interthane') || name.includes('beckers') || name.includes('domalux') || name.includes('optiva') || name.includes('emul.semilatex')) {
      newCatSlug = 'farby-wewnetrzne';
      newRootSlug = 'farby-i-rozpuszczalniki';
    } else if (name.includes('rega ekspozycyjny')) {
      newCatSlug = 'wiadra-i-pojemniki-budowlane';
      newRootSlug = 'narzedzia-i-mocowania';
    } else if (name.includes('schlicht a2')) {
      newCatSlug = 'plyty-sufitowe';
      newRootSlug = 'sufity-podwieszane';
    } else if (name.includes('pilmas') || name.includes('wodorotlenek sodu') || name.includes('esco-fluat') || name.includes('kolorant')) {
      newCatSlug = 'chemia-budowlana';
      newRootSlug = 'chemia-budowlana';
    } else if (name.includes('kołek szybkiego montażu') || name.includes('kołnierz docisk') || name.includes('zaślepka do prof') || name.includes('złączka 120') || name.includes('dysza')) {
      newCatSlug = 'elementy-mocujace-uniwersalne';
      newRootSlug = 'narzedzia-i-mocowania';
    } else if (name.includes('kominek bergamo') || name.includes('osłona went')) {
      newCatSlug = 'akcesoria-do-kominow';
      newRootSlug = 'systemy-kominowe';
    } else if (name.includes('otwieracz do kartuszy')) {
      newCatSlug = 'narzedzia-reczne';
      newRootSlug = 'narzedzia-i-mocowania';
    } else if (name.includes('zaprawa cementowa')) {
      newCatSlug = 'zaprawy';
      newRootSlug = 'chemia-budowlana';
    } else if (name.includes('krzyżyki dyst')) {
      newCatSlug = 'narzedzia-i-akcesoria-glazurnicze';
      newRootSlug = 'plytki';
    } else if (name.includes('klin z wełny')) {
      newCatSlug = 'welny-fasadowe';
      newRootSlug = 'izolacje-fasadowe';
    } else if (name.includes('kostka bruk')) {
      newCatSlug = 'galanteria-betonowa';
      newRootSlug = 'stropy-i-sciany';
    } else if (name.includes('masa do spoinowania')) {
      newCatSlug = 'masy-szpachlowe-gotowe';
