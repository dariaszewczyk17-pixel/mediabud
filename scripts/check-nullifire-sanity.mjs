...
async function check() {
  const product = await client.fetch(`*[_type == "product" && slug.current == "nullifire-sc803-25-kg"]{
    name, "slug": slug.current, _id, sku, "category": category->slug.current, "rootCategory": rootCategory->slug.current
  }`);
  
  console.log("Status produktu 'nullifire-sc803-25-kg' w Sanity:");
