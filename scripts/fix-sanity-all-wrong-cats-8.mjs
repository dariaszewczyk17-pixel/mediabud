...
async function fix() {
  ...
  for (const p of products) {
    ...
    if (name.includes('rękawic') || name.includes('rekawic')) {
      ...
      newCatSlug = 'uszczelki-do-okien-i-drzwi';
      newRootSlug = 'stolarka-okienna-i-drzwiowa';
    }
    
    if (newCatSlug && newRootSlug) {
      // Szukamy kategorii po slugu, ignorując prefiksy cat-l3- itp.
      const cat = categories.find(c => c.slug === newCatSlug || c.slug.endsWith('-' + newCatSlug));
      const root = categories.find(c => c.slug === newRootSlug || c.slug.endsWith('-' + newRootSlug));
